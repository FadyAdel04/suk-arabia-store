            ...
            <Carousel className="relative">
              <CarouselContent>
                {products.map((product, index) => (
                  <CarouselItem
                    key={product.id}
                    className="!flex items-stretch justify-center px-2 md:basis-1/2 lg:basis-1/4 w-full max-w-sm mx-auto"
                  >
                    <Card className="group hover:shadow-xl transition-all duration-300 animate-fade-in hover-scale overflow-hidden flex flex-col h-full">
                      <div className="relative">
                        <Link to={`/product/${product.id}`}>
                          <img
                            src={product.image_url || "/placeholder.svg"}
              ...
              {/* Overlay nav buttons for carousel, always visible, improve mobile placement */}
              <div className="pointer-events-none">
                <div className="hidden md:block">
                  <CarouselPrevious className="left-2 top-1/2 -translate-y-1/2 z-10 pointer-events-auto" />
                  <CarouselNext className="right-2 top-1/2 -translate-y-1/2 z-10 pointer-events-auto" />
                </div>
                {/* Mobile buttons at the bottom */}
                <div className="absolute flex md:hidden left-1/2 -translate-x-1/2 bottom-[-28px] z-10 gap-3">
                  <CarouselPrevious className="relative left-0 top-0 pointer-events-auto" />
                  <CarouselNext className="relative right-0 top-0 pointer-events-auto" />
                </div>
              </div>
            </Carousel>
          </div>
